...
class postController extends Controller //V5
{
    ...
    public function index($pagina = false)                                //V12
    {
        //for($i = 0; $i < 300; $i++){                                       //V12
        //    $model = $this->loadModel('post');                             //V12
        //    $model->insertarPost('titulo' . $i, 'cuerpo' . $i);            //V12
        //}                                                                  //V12

        if(!$this->filtrarInt($pagina)){                                     //V12
            $pagina = false;                                                 //V12
        }                                                                    //V12
        else {                                                               //V12
            $pagina = (int) $pagina;                                         //V12
        }                                                                    //V12

        $this->getLibrary('paginador','paginador');                        //V12
        $paginador = new Paginador();                                      //V12
        //$this->_view->posts = $this->_post->getPosts();                    //V5
        $post = $this->loadModel('post');                                  //V12
        $this->_view->posts = $paginador->paginar($this->_post->getPosts(), $pagina); //V12
        $this->_view->paginacion = $paginador->getView('prueba', 'post/index');       //V12 
    ...
    public function nuevo()                                                 //V5
    {
        ...
        //Session::accesoEstricto(array('usuario'),true);                            //V8
        Session::accesoEstricto(array('usuario'));                            //V8
    ...
    public function prueba($pagina = false)                    //V12
    {
        /*
        for($i =0; $i <300; $i++){                              //V12
            $model = $this->loadModel('post');                  //V12
            $model->insertarPrueba('nombre ' . $i) ;            //V12
        }                                                       //V12
        */
        if(!$this->filtrarInt($pagina)){                        //V12
            $pagina = false;                                    //V12
        ...
        $this->_view->paginacion = $paginador->getView('prueba', 'post/prueba'); // llama a la paginacion relacionada con la vista //V12
        $this->_view->titulo = 'post';                          //V12
        $this->_view->renderizar('prueba', 'post');             //V12
    }
