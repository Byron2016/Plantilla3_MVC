...
class menuController extends Controller                                   //V5
{
    ...
    public function getMenu()                                             //V5
    {
        ...
        for ($i = 0; $i < count($menu_aux); $i++) {                       //V5
            ...
            if (($menu_aux[$i]['id_negocio'] == "Login_Autenticado" 
                || $menu_aux[$i]['id_negocio'] == "Login_No_Autenticado" 
                || $menu_aux[$i]['id_negocio'] == "registro")) {                                            //V10
                if ($ingresar) {
                    if (Session::get('autenticado')) {                                                      //V10
                        if ($menu_aux[$i]['id_negocio'] == "Login_Autenticado" ) {                          //V10
                            $menu_def[$j]['id'] = $menu_aux[$i]['id_negocio'];                              //V10
                            $menu_def[$j]['titulo'] = $menu_aux[$i]['titulo'];                              //V10
                            $menu_def[$j]['enlace'] = BASE_URL . $menu_aux[$i]['enlace'];                   //V10
                            $menu_def[$j]['image'] = $menu_aux[$i]['image'];                                //V10
                            $menu_def[$j]['logeado'] = $menu_aux[$i]['logeado'];                            //V10
                            $ingresar = FALSE;                                                              //V10
                            $j++;                                                                           //V10
                        }                                                                                   //V10
                    } else {                                                                                //V10
                        if ($menu_aux[$i]['id_negocio'] == "Login_No_Autenticado" ) {                       //V10
                            $menu_def[$j]['id'] = "Login_No_Autenticado";                                   //V10
                            $menu_def[$j]['titulo'] = $menu_aux[$i]['titulo'];                              //V10
                            $menu_def[$j]['enlace'] = BASE_URL . $menu_aux[$i]['enlace'];                   //V10
                            $menu_def[$j]['image'] = $menu_aux[$i]['image'];                                //V10
                            $menu_def[$j]['logeado'] = $menu_aux[$i]['logeado'];                            //V10
                ...
                if (!Session::get('autenticado') && $menu_aux[$i]['id_negocio'] == "registro") {            //V10
                    $menu_def[$j]['id'] = $menu_aux[$i]['id_negocio'];                                      //V10
                    $menu_def[$j]['titulo'] = $menu_aux[$i]['titulo'];                                      //V10
                    $menu_def[$j]['enlace'] = BASE_URL . $menu_aux[$i]['enlace'];                           //V10
                    $menu_def[$j]['image'] = $menu_aux[$i]['image'];                                        //V10
